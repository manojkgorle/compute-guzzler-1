...
class StepMetrics:
    ...
    mlp_output_norms: list[float] | None = None
    mlp_sparsity: list[float] | None = None
...
class MetricsLogger:
    ...
    def log_step(self, metrics: StepMetrics) -> None:
        """Append a training step to the in-memory deque and write to JSONL."""
    ...
    def log_validation(self, metrics: ValMetrics) -> None:
        """Append a validation result to the in-memory deque and write to JSONL."""
        record = asdict(metrics)
        with self._lock:
    ...
    def get_all_steps(self) -> list[dict]:
        """Return all training step metrics as a list of dicts."""
        with self._lock:
            return list(self._steps)

    def get_all_validations(self) -> list[dict]:
        """Return all validation metrics as a list of dicts."""
        with self._lock:
            return list(self._validations)

    def get_steps_since(self, step: int) -> list[dict]:
        """Return training metrics for steps strictly greater than the given step."""
        with self._lock:
            return [s for s in self._steps if s["step"] > step]

    def load_from_file(self) -> None:
        """Load existing JSONL files into the in-memory deques."""
        with self._lock:
            self._steps.clear()
